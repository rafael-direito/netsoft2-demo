...
network_info = os.getenv('tunnel_bandwidth_network_info')
...
def tunnelbandwidth():
    ...
    time.sleep(5)
    stdin, stdout, stderr = machine2.exec_command(f"sudo iperf3 -c {target} --json -t 5")
    ...
    if len(bits_per_second_results) > 0: 
        # Calculate the packet loss mean between Sender and Receiver
